...
```{r}
  ...
  hist(before,main = paste("Before"),freq = T, xlab = "Values");      lines(density(before))
  qqnorm(before, main = paste("Before"))
  qqline(before,col=2,lwd=2)
  abline(h=median(before))
  abline(h=mean(before),col=3)
  boxplot(before, horizontal = T, main = paste("Before"))
  
```
```{r}
 hist(after,main = paste("After"),freq = T, xlab = "Values");      lines(density(before))
  qqnorm(after, main = paste("After"))
  qqline(after,col=2,lwd=2)
  abline(h=median(after))
  abline(h=mean(after),col=3)
  boxplot(after, horizontal = T, main = paste("After"))
```
...
```
Der Resultat zeigt, dass die Test-Statistik einen Wert von t = 4.9322 hat und aufgrund des p-values von 0.0008109 die Nullhypothese (es gibt keinen Effekt) sowohl auf 5% als auch auf 1% Signifikanzniveau verworfen werden kann. Das Training hatte daher einen Effekt. 
...
```
Der p-Value bei diesem t-Test beträgt 0.0004055, wodurch auch hier die Nullhypothese sowohl auf 5% als auch auf 1% Signifikanzniveau verworfen werden kann. 
