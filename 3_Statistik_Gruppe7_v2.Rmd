...
Bayes factors
The Bayes Factor is the ratio of posterior probabilites of parameters under 2 hypotheses.
BF =
P[θ ∈ Θ0|x]
P[θ ∈ Θ1|x]
This is symmetric w.r.t.˜the two hypotheses and on ratio scale, thus BF = 2 can be interpreted as
H0 is twice as likely as H1. If you want asymmetry you can add weights (i.e.“loss’ ’ when making the
wrong decision). The decision is then based on k0
k1
· B
Im Zuge der Überarbeitung haben wir uns dazu entschieden, einen Hypothesentest mit dem Bayesfaktor durchzuführen. Dazu haben wir das Paket BayesFactor benutzt. Dabei wird ausgehend von der Verteilung ein Bayesfaktor erstellt, der der Quotienten  P(h0) / P(h1) ist.
H0 ist hierbei, dass der wahre Mittelwert der Daten zwischen 0.01 und 0.01 liegt, H1 ist, dass der wahre Mittelwert ausserhalb davon liegt.

1] Alt., r=0.707 : 5641.645 ±0% spricht dafür, dass der wahre Mittelwert mit hoher wahrscheinlichekit nicht bei 0.01 liegt. Dadruch decken sich hier unsere Ergebnise mit der Ausarebitung aus dem WS, bei der wir aufgrund des student t-Tests davon ausgegangen sind, 


```{r}

daten <- c(0.146842, 0.156757, 0.091255, 0.063720, 0.148471, 
                    -0.045436, 0.150407, 0.077905, 0.077267, 0.026454, 
                    0.090700, 0.245384, 0.129650, 0.141617, 0.039957, 
                    0.165351, 0.029091, 0.073473, 0.189657, 0.123897)
library(BayesFactor)
...
Da es sich hierbnei um nicht parametrische Daten handelt und die Verteilung der Daten unterschiedlich ist, und da firma 2 starke ausreißer beinhaltet, kann man hier nicht einfach einenWilcoxon test durchführen. Stattdessen haben wir uns dazu entschieden, eine Bootsreapping Simulation mit Resampling durchzuführen. Dazu haben wir wieder n=1000 genommen und die Erwartete Differenz des Mittelwerts berechnet.
